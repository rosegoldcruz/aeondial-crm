...
export default function CalendarsPage() {
  ...
  const mobileYear = today.getFullYear()
  const mobileMonth = today.getMonth()
  const mobileStartDay = new Date(mobileYear, mobileMonth, 1).getDay()
  const mobileTotalDays = new Date(mobileYear, mobileMonth + 1, 0).getDate()
  const MOBILE_HEADERS = ['S','M','T','W','T','F','S']
  ...
  return (
    <div className="h-full" style={{ background: 'var(--cyber-bg-darkest)', color: 'var(--cyber-text-primary)' }}>
      ...
      {/* ========== MOBILE VIEW ========== */}
      <div className="md:hidden min-h-screen bg-[#0a0a0a] px-2 pt-4 pb-24">
        {/* Month header */}
        <div className="flex items-center justify-between px-2 mb-4">
          <span className="text-white font-bold text-lg">
            {today.toLocaleString('default',{month:'long'})} {mobileYear}
          </span>
          <button
            onClick={() => setShowCreateModal(true)}
            className="w-8 h-8 bg-[#22d3ee] rounded-full flex items-center justify-center text-white text-xl leading-none"
          >+</button>
        </div>

        {/* Day headers */}
        <div className="grid grid-cols-7 mb-1">
          {MOBILE_HEADERS.map((h,i) => (
        ...
        <div className="grid grid-cols-7 gap-px bg-[#262626] rounded-xl overflow-hidden">
          {Array.from({length: mobileStartDay}).map((_,i) => (
            <div key={'e'+i} className="bg-[#0a0a0a] h-12" />
          ))}
          {Array.from({length: mobileTotalDays}).map((_,i) => {
            const day = i + 1
            const isMobileToday = day === today.getDate()
            const isMobileSelected = day === mobileSelected
            return (
              <div
                key={day}
                onClick={() => setMobileSelected(day)}
                className="bg-[#0a0a0a] h-12 flex flex-col items-center justify-start pt-1 active:bg-[#1a1a1a] transition-colors cursor-pointer">
                <span className={`text-xs w-6 h-6 flex items-center justify-center rounded-full font-medium ${isMobileToday ? 'bg-[#22d3ee] text-white' : isMobileSelected ? 'bg-[#1a1a1a] text-white' : 'text-[#a3a3a3]'}`}>
                  {day}
                </span>
              </div>
            )
        ...
        {mobileSelected && (
          <div className="mt-4 mx-2 bg-[#111111] border border-[#262626] rounded-xl p-4">
            <p className="text-xs text-[#525252] uppercase tracking-wider">
              {today.toLocaleString('default',{month:'long'})} {mobileSelected}
            </p>
